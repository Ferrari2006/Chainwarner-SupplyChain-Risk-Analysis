...
class GraphEngine:
    ...
    def calculate_metrics(self):
        ...
        # OPTIMIZATION: Skip constraint on large graphs or lite mode (it's O(V^3) or O(VE))
        if not lite_mode and node_count < 100:
            try:
            ...
            # Fast Fallback for Lite Mode
            metrics['constraint'] = {n: (self.G.degree(n) / max(1, len(self.G) - 1)) for n in self.G.nodes}

        # 2. Betweenness Centrality
        # Identifies nodes that control information flow.
        # OPTIMIZATION: Skip betweenness on large graphs (O(VE))
        if not lite_mode and node_count < 200:
            try:
             ...
             metrics['betweenness'] = {n: 0.0 for n in self.G.nodes}
