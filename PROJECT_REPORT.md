...
### 3.5 多项目深度对比 (Deep Comparison)
**场景**：用户通过 API 对比两个项目（如 `Django` vs `FastAPI`）。
**功能**：
*   **全维度 PK**：直接对比风险评分、活跃度、OpenRank 及 **真实 CVE 漏洞数**。
*   **自动胜负判定**：基于加权风险模型自动判定“更安全”的项目。

### 3.6 自动化安全审计报告 (Auto Audit Report)
**场景**：生成项目体检单。
**功能**：
*   **CVE 漏洞集成**：接入 **OSV.dev** 全球漏洞数据库，实时扫描已知漏洞。
*   **依赖链体检**：自动识别依赖树中的高危组件并给出修复建议。
...
2.  **全球漏洞库实时集成 (Real-time CVE Integration)**
    *   **技术**: 无缝对接 **OSV.dev** API，实现 Python (PyPI) 和 JavaScript (npm) 生态的精准漏洞查询。
    *   **优势**: 告别模拟数据，提供真实可信的供应链安全情报。

3.  **低资源消耗的混合智能体 (Hybrid Agent)**
    *   **技术**: 独创“动态模板注入”技术，在没有 GPU 和 API Key 的边缘设备上也能提供类 AI 的交互体验。
    *   **优势**: 极大降低了部署门槛和运行成本，适合大规模推广。

4.  **EasyGraph 深度融合 (Graph Intelligence)**
...
3.  **[已完成] 生态对比功能**: 上线双项目 PK 模块及自动化审计报告。
4.  **[已完成] 漏洞库集成**: 接入 OSV.dev 实现真实 CVE 扫描。
5.  **[计划中] 多源数据融合**: 计划接入 Gitee、GitLab 等更多代码托管平台的数据。
